...
int main()
{
	...
	do {
		...
		cout << " [1] - створення файла студент" << endl;
		cout << " [2] - вивести на екран" << endl;
		cout << " [3] - Кількість оцінок з програмування «5», «4», «3»:" << endl;
		cout << " [4] - Відсоток студентів:  " << endl;
...
Student Create()
{
	...
	cout << "оцінки з інформатики: "; cin >> p.grades_in_informatic;
	return p;
}
...
void Print_student(int N, Student student)
{
	...
	cout << "|" << setw(20) << right << student.grades_in_informatic << " " << " |" << endl;
}
void Print(char* fname)
{
	ifstream fin(fname);
...
void LineSearch_count(char* fname, int* count_3, int* count_4, int* count_5)
{
	...
	while (fin.read((char*)&student, sizeof(Student)))
	{
		if (student.grades_in_informatic == 3)
		{
			(*count_3)++;
		}
		else if (student.grades_in_informatic == 4)
		{
			(*count_4)++;
		}
		else if (student.grades_in_informatic == 5)
		{
			(*count_5)++;
		}
